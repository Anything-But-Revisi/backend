...
- **WHEN** `/api/openapi.json` is accessed
- **THEN** session endpoints are documented with correct paths, methods, parameters

#### Scenario: Swagger UI integration
- **WHEN** `/api/docs` is accessed
- **THEN** users can see and test session endpoints interactively

#### Scenario: Endpoint tags
- **WHEN** endpoints are registered
- **THEN** they are tagged as "Sessions" or similar for documentation organization

### Requirement: Request/Response Logging
The system SHALL log endpoint usage for monitoring.

#### Scenario: Log successful requests
- **WHEN** endpoints complete successfully
- **THEN** info-level logs record method, path, status code, and timing

#### Scenario: Log errors
- **WHEN** endpoints fail
- **THEN** warning/error logs record method, path, error code, and error details

#### Scenario: No sensitive data in logs
- **WHEN** logs are written
- **THEN** session IDs are included but no user data or PII appears
